...
4. Decorate the action with the **Trigger** attribute passing the argument **TriggerType.Poll** to the constructor, as well as the type of model that will be sent when data is available (e.g., **typeof(MyModelClassHere)**)
5. Make sure the action has a **string** parameter named **triggerState**
...
6. Make sure that the action returns the value generated by calling **Request.EventWaitPoll** when no data is available
...
7. Make sure that the action returns the value generated by calling **Request.EventTriggered** when data is available
