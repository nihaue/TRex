...
7. Make sure the action has a parameter named **parameters** of type **`TriggerInput<TInput,TOutput>`** if the trigger will require configuration data to be entered by the user of the Logic App triggered by it, or **`TriggerInput<TOutput>`** if not.
